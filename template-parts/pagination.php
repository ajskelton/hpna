...
$prev_text = sprintf(
	...
	__( '<span class="nav-short">Prev</span>', 'hpna' )
);
$next_text = sprintf(
	'<span class="nav-next-text">%s</span>',
	__( '<span class="nav-short">Next</span>', 'twentytwenty' )
);
...
if ( strpos( $posts_pagination, 'prev page-numbers' ) === false ) {
	$posts_pagination = str_replace( '<div class="nav-links">', '<div class="nav-links"><span class="prev page-numbers disabled" aria-hidden="true">' . $prev_text . '</span>', $posts_pagination );
}

// If we're not outputting the next page link, append a placeholder with visibility: hidden to take its place.
if ( strpos( $posts_pagination, 'next page-numbers' ) === false ) {
	$posts_pagination = str_replace( '</div>', '<span class="next page-numbers disabled" aria-hidden="true">' . $next_text . '</span></div>', $posts_pagination );
}
